...
TOMCytos = exportNetworkToCytoscape(modTOM, edgeFile = paste("edgesTOM_sft", sft$powerEstimate, "_ModSize", minModuleSize, "_cutHeight", cutHeight, "_deepSplit", deepSplit, "_", selModules, ".txt", sep=""),
nodeFile = paste("nodesTOM_sft", sft$powerEstimate, "_ModSize", minModuleSize, "_cutHeight", cutHeight, "_deepSplit", deepSplit, "_", selModules, ".txt", sep=""), 
weighted = TRUE, threshold = adjThreshold, nodeNames = modGenes, altNodeNames = modGenes, nodeAttr = mergedColors[inModule])
